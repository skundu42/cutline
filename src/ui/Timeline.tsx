...
function linkedItem(branch: ReturnType<typeof activeBranch>, item: ClipInstance) {
  if (item.trackId !== "v1") return null;
  return branch.tracks.find((track) => track.trackId === "a1")?.items.find((candidate) =>
    candidate.assetId === item.assetId
    && candidate.startMs === item.startMs
    && candidate.endMs === item.endMs
    && candidate.sourceInMs === item.sourceInMs,
  ) ?? null;
}
...
export function Timeline() {
  ...
  const selectedLinkedItem = selectedItem ? linkedItem(branch, selectedItem) : null;
  const canSplit = Boolean(selectedItem && playheadMs > selectedItem.startMs && playheadMs < selectedItem.endMs);
  ...
  useEffect(() => {
    ...
    const onMove = (event: PointerEvent) => {
      ...
      if (drag.mode === "move") {
        const startMs = Math.max(0, snap(drag.item.startMs + deltaMs));
    ...
    const onUp = () => {
      const current = drag;
      const linked = linkedItem(branch, current.item);
      const operations: EditOp[] = [];
      if (current.mode === "move" && current.previewStartMs !== current.item.startMs) {
        operations.push({ op: "move", itemId: current.item.itemId, startMs: current.previewStartMs });
        if (linked) operations.push({ op: "move", itemId: linked.itemId, startMs: current.previewStartMs });
      }
      if (current.mode !== "move" && (current.previewStartMs !== current.item.startMs || current.previewEndMs !== current.item.endMs)) {
        ...
        if (linked) operations.push({ op: "trim", itemId: linked.itemId, startMs: current.previewStartMs, endMs: current.previewEndMs });
      }
      if (operations.length) {
        ...
        setSelectedRange({ startMs: current.previewStartMs, endMs: current.previewEndMs });
      }
      setDrag(null);
    };
  ...
  }, [branch, dispatch, drag, frameMs, setSelectedRange, timelineWidth]);
  ...
  const splitSelected = () => {
    ...
    if (selectedLinkedItem) operations.push({ op: "split", itemId: selectedLinkedItem.itemId, atMs: Math.round(playheadMs) });
    dispatch({ type: "ApplyEditBatch", actor: { type: "human", surface: "ui" }, payload: { branchId: branch.branchId, expectedBranchVersion: branch.branchVersion, operations } });
    setSelectedItemId(null);
  };
  ...
  const deleteSelected = () => {
    ...
    const operations: EditOp[] = [{ op: "delete", itemId: selectedItem.itemId }];
    if (selectedLinkedItem) operations.push({ op: "delete", itemId: selectedLinkedItem.itemId });
    dispatch({ type: "ApplyEditBatch", actor: { type: "human", surface: "ui" }, payload: { branchId: branch.branchId, expectedBranchVersion: branch.branchVersion, operations } });
    setSelectedItemId(null);
    setSelectedRange(null);
  };
    ...
    <section className="timeline-panel" aria-label="Timeline editor">
      <div className="timeline-header">
        ...
        <div className="timeline-edit-actions" aria-label="Selected clip actions">
          ...
          <button data-testid="split-selected" type="button" disabled={!canSplit} title={selectedItem ? "Move the playhead inside the clip to split it (S)" : "Select a clip first"} onClick={splitSelected}>Split</button>
          <button type="button" disabled={!selectedItem} title="Delete the clip and close the gap" onClick={rippleSelected}>Ripple</button>
          <button type="button" disabled={!selectedItem} onClick={deleteSelected}>Delete</button>
        </div>
        ...
        <button data-testid="lock-range" className="timeline-action" disabled={!selectedRange} title={selectedRange ? "Protect the selected range (L)" : "Select a transcript segment or timeline clip first"} onClick={() => { if (selectedRange) dispatch({ type: "SetLock", actor: { type: "human", surface: "ui" }, payload: { action: "lock", branchId: branch.branchId, expectedBranchVersion: branch.branchVersion, range: selectedRange, label: "Keep pause" } }); }}><LockIcon /> Protect <kbd>L</kbd></button>
              ...
              <div key={track.trackId} className="timeline-track" style={{ gridTemplateColumns: `${LABEL_WIDTH}px ${timelineWidth}px` }}>
                ...
                <div className="track-lane" style={{ width: timelineWidth }}>
                  {track.items.map((item) => {
                    ...
                    return <div key={item.itemId} role="button" tabIndex={0} aria-label={`${item.label}, ${formatTimecode(item.startMs)} to ${formatTimecode(item.endMs)}`} title={`${item.label} ${formatTimecode(item.startMs)}–${formatTimecode(item.endMs)} · drag to move`} className={`timeline-clip ${TRACK_COLORS[track.trackId] ?? "track-video"} ${selectedItemId === item.itemId ? "is-selected" : ""} ${dragged ? "is-dragging" : ""}`} aria-pressed={selectedItemId === item.itemId} style={{ left: px(shownStart), width: Math.max(4, px(shownEnd) - px(shownStart)) }} onPointerDown={(event) => beginDrag(event, item, "move")} onClick={() => { setSelectedRange({ startMs: item.startMs, endMs: item.endMs }); setSelectedItemId(item.itemId); setPlayhead(item.startMs); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedRange({ startMs: item.startMs, endMs: item.endMs }); setSelectedItemId(item.itemId); setPlayhead(item.startMs); } }}><button className="trim-handle is-start" aria-label={`Trim start of ${item.label}`} onPointerDown={(event) => beginDrag(event, item, "trim-start")} /><span>{item.label}</span><button className="trim-handle is-end" aria-label={`Trim end of ${item.label}`} onPointerDown={(event) => beginDrag(event, item, "trim-end")} /></div>;
                  })}
      ...
      <div className="timeline-help">Space play · ← → frame step · S split · drag clips to move · drag edges to trim</div>
